...
const App = () => {
  ...
  const handleButtonClick = async (infoType) => {
    ...
    if (infoType === "방문 예약") {
      ...
      } else if (infoType === "대국민 공모 신청") {
        // 대국민 공모 신청 버튼 클릭 시 고정 멘트와 링크 제공
        newMessages = [
          ...messages,
          {
            type: "bot",
            text: (
              <>
                대국민 공모 신청 서비스에 접수하기 위해서 아래 사이트에 접속해
                접수해주기 바랍니다.
                <br />
                <a
                  href="http://localhost:8000/contest-entry"
                  target="_blank"
  ...
  const handleSend = async () => {
    if (input.trim() !== "") {
      ...
      if (
        lowercaseInput.includes("방문예약") ||
        lowercaseInput.includes("출입증 등록")
      ) {
        const formMessage = {
          type: "bot",
