...
const convertTextToLinks = (text) => {
  ...
  return text.split(urlPattern).map((part, index) => {
    if (urlPattern.test(part)) {
      // 링크가 특정 URL일 경우, 보이는 링크와 실제 링크를 다르게 설정
      const displayText = "https://port.com/contest-entry"; // 사용자에게 보여질 URL
      const actualLink = "http://localhost:8000/contest-entry"; // 실제로 이동할 서버 URL
        ...
        <div className={`chat-message bot`}>
          ...
          <div className="message-container bot">
            <div className="message-bubble">
              ...
              <p>무엇을 도와드릴까요?</p>
              <p>
                키워드 입력보다는 대화형으로 문의하시면 더 잘 알려드릴 수
                있어요!
              </p>
            </div>
          ...
          <div className="buttons-outside">
            ...
            <div className="buttons-row">
              ...
              <button
                onClick={() => handleButtonClick("service_intro")}
                className="wide-button"
              >
                <div
                  style={{
                ...
                <div className="message-bubble">
                  {message.isIframe ? (
                    ...
                    message.text.map((line, index) => (
                      <p key={index}>{convertTextToLinks(line)}</p>
                    ))
                  ) : (
